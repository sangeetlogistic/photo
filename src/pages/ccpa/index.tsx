...
import { selectCcPAPrivacy, selectError, selectLoading, setCcpaDetail } from '../../services/API/GeneralSettings/GeneralSettings.slice';
...
const CCPA = ({ seoResult, ccpaResult }: any) => {
    ...
    useEffect(() => {
        dispatch(setCcpaDetail(ccpaResult));
    }, [ccpaResult]);
    ...
    return (
        <>
            <Head>
                ...
                <meta name="twitter:data1" content={`${ccpaResult?.readingTime} minutes`} />
                ...
                <CommonPolicyCmp>
                    ...
                    <Content className="container">
                        <div className="cookie_section">
                            ...
                            <div className="editor_content_cookie">{parse(ccpaResult?.detail?.data?.[0]?.description || '')}</div>
                        </div>
                    </Content>
                </CommonPolicyCmp>
            </LayoutCmp>
...
export const getServerSideProps = async (context) => {
    ...
    try {
        const response: any = await GeneralSettings.cCPAPrivacyNotice();
        if (response.status === statusCode.success) {
            ccpaResult = { detail: response.data, readingTime: getReadContentTime(response.data?.data?.[0]?.description) };
        }
    } catch (err: any) {
        if (!err.response) {
            throw err;
        }
        ccpaResult = { error: { message: err?.response?.data?.message || null, code: err?.response?.data?.code || null } };
    }
