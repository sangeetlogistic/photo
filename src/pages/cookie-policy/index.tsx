...
import { selectCookiePolicy, selectError, setCookieDetail } from '../../services/API/GeneralSettings/GeneralSettings.slice';
...
const CookiePolicy = ({ seoResult, cookieResult }: any) => {
    ...
    useEffect(() => {
        dispatch(setCookieDetail(cookieResult));
    }, [cookieResult]);
    ...
    return (
        <>
            <Head>
                ...
                <meta name="twitter:data1" content={`${cookieResult?.readingTime} minutes`} />
                ...
                <CommonPolicyCmp>
                    ...
                    <Content className="container">
                        <div className="cookie_section">
                            ...
                            <div className="editor_content_cookie">{parse(cookieResult?.detail?.data?.[0]?.description || '')}</div>
                        </div>
                    </Content>
                </CommonPolicyCmp>
            </LayoutCmp>
...
export const getServerSideProps = async (context) => {
    ...
    let cookieResult: any = {};

    try {
        const response: any = await GeneralSettings.cookiePolicy();
        if (response.status === statusCode.success) {
            cookieResult = { detail: response.data, readingTime: getReadContentTime(response.data?.data?.[0]?.description) };
        }
    } catch (err: any) {
        if (!err.response) {
            throw err;
        }
        cookieResult = { error: { message: err?.response?.data?.message || null, code: err?.response?.data?.code || null } };
    }
